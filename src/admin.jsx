...
const [destinationTypes, setDestinationTypes] = useState([])
  ...
  async function saveBanner(e) {
    ...
    try {
      ...
      if (localMode) { const current = localSeed() || {}; const banners = current.home?.banners || []; localUpdate('home', (value = {}) => ({ ...value, banners: isEdit ? banners.map((item) => item.id === editing ? { ...item, ...payload, id: editing, updatedAt: new Date().toISOString() } : item) : [...banners, { ...payload, id: localId('banner'), updatedAt: new Date().toISOString() }] })) }
  ...
  async function updateBannerStatus(itemId, status) {
    ...
    try { if (localMode) localUpdate('home', (current = {}) => ({ ...current, banners: (current.banners || []).map((item) => item.id === itemId ? { ...item, ...payload, updatedAt: new Date().toISOString() } : item) })); else await callApi(`/admin/miniprogram-home-banners/${itemId}`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify(payload) }); await load(); notify('Banner 状态已更新') } catch (error) { notify(error.message) }
  }
  async function removeBanner(itemId) {
    if (!window.confirm('确定删除这张小程序首页 Banner 吗？删除后不会影响 Website 首页。')) return
    try { if (localMode) localUpdate('home', (current = {}) => ({ ...current, banners: (current.banners || []).filter((item) => item.id !== itemId) })); else await callApi(`/admin/miniprogram-home-banners/${itemId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }); await load(); notify('小程序首页 Banner 已删除') } catch (error) { notify(error.message) }
  }
  async function moveBanner(item, offset) {
    ...
    try {
      if (localMode) localUpdate('home', (current = {}) => ({ ...current, banners: next.map((entry, entryIndex) => ({ ...entry, sort: entryIndex + 1, updatedAt: new Date().toISOString() })) }))
  ...
  async function saveHomeBanners(next) {
    ...
    try {
      if (localMode) localUpdate('settings', (current) => ({ ...current, homeBanners: next }))
