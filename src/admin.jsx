...
import { BarChart3, Check, ChevronRight, FileText, Globe2, LogOut, MapPinned, Plus, Save, Settings, Trash2, Users, X } from 'lucide-react'
...
export default function AdminPage() {
  ...
  const [stats, setStats] = useState({ routes: 0, destinations: 0, leads: 0, pendingLeads: 0 })
  ...
  function applyData(data) { setRoutes(data.routes || []); setDestinations(data.destinations || []); setLeads(data.leads || []); setSettings(data.settings || settings); setStats({ routes: (data.routes || []).length, destinations: (data.destinations || []).length, leads: (data.leads || []).length, pendingLeads: (data.leads || []).filter((lead) => lead.status === 'new').length }) }
  async function load() {
    if (localMode) return applyData(localData)
    try {
      const auth = { Authorization: `Bearer ${token}` }
      const [nextStats, nextRoutes, nextDestinations, nextLeads, nextSettings] = await Promise.all(['/admin/stats', '/admin/routes', '/admin/destinations', '/admin/leads', '/admin/settings'].map((path) => callApi(path, { headers: auth })))
      setStats(nextStats); setRoutes(nextRoutes); setDestinations(nextDestinations); setLeads(nextLeads); setSettings(nextSettings)
