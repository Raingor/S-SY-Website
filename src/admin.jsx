...
const defaultSettings = { siteName: '', siteUrl: '', defaultTitle: '', defaultDescription: '', keywords: '', ogImage: '', googleVerification: '', robotsPolicy: 'index,follow', wechat: '', phone: '', email: '', replyHours: '', miniprogramAccess: true, miniprogramKnowledge: { trialSeconds: 60, products: { attraction: { enabled: true, productType: 'attraction', name: '单景点永久讲解（模拟）', price: 1, currency: 'CNY' }, membership: { enabled: true, productType: 'membership', name: '终身会员（模拟）', price: 1, currency: 'CNY' } } } }
...
function MiniKnowledgeSettings({ value, onChange }) {
  ...
  const products = { attraction: { name: '单景点永久讲解（模拟）', price: 1, currency: 'CNY', enabled: true, ...(config.products?.attraction || {}) }, membership: { name: '终身会员（模拟）', price: 1, currency: 'CNY', enabled: true, ...(config.products?.membership || {}) } }
