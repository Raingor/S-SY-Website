...
const emptyAttraction = { name: '', en: '', originalName: '', city: 'athens', cityName: '雅典', type: 'landmark', category: '', sizeLabel: '', tags: '', image: 'athens.webp', summary: '', status: 'published', exhibits: [] }
...
const formTemplates = {
  ...
  itinerary: { title: '参考行程填写模板', intro: '参考行程用于展示整体节奏，逐日结构化内容可继续通过数据维护。', fields: [['行程名称', '雅典深度文化 5 日'], ['天数', '5'], ['简介', '概括城市、主题和适合的旅行节奏。'], ['封面图片', '上传行程封面图']] },
...
export default function AdminPage() {
  ...
  async function saveAttraction(e) {
    e.preventDefault()
    const isEdit = editing && editing !== 'new'
    const payload = { ...attractionForm, tags: String(attractionForm.tags || '').split(/[,，、\s]+/).filter(Boolean), exhibits: normalizeExhibits(attractionForm.exhibits) }
    try {
