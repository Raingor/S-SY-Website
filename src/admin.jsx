...
import { ArrowLeft, BarChart3, CalendarDays, Check, ChevronDown, ChevronRight, ChevronUp, Copy, CreditCard, FileText, Globe2, Images, Landmark, Link2, LogOut, MapPinned, Plus, RefreshCw, Save, Settings, Smartphone, Trash2, Users, X } from 'lucide-react'
...
function ImageUploadField({ label, value, onChange, onUpload, required = false, hint = '支持 PNG、JPG、WebP，单张不超过 6MB' }) {
  ...
  return <div className={`admin-image-field ${dragging ? 'is-dragging' : ''}`} onDragEnter={() => setDragging(true)} onDragLeave={() => setDragging(false)} onDragOver={(event) => event.preventDefault()} onDrop={handleDrop} aria-busy={uploading}><span>{label}</span>{value ? <div className="admin-image-preview"><img src={assetPath(value)} alt={label + '预览'} /><span>当前图片</span></div> : <div className="admin-image-empty">{dragging ? '松开以上传图片' : '尚未选择图片'}</div>}<div className="admin-image-controls"><label className="admin-file-picker"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={handleChange} disabled={uploading} required={required && !value} /><strong>{uploading ? '上传中…' : value ? '重新选择图片' : '选择图片上传'}</strong></label>{value && <button type="button" className="admin-image-remove" onClick={() => onChange('')} disabled={uploading}>删除图片</button>}</div><small>{hint} · 也可将图片拖到此处</small></div>
}

function MiniProgramBannersPanel({ items = [], editing, setEditing, form, setForm, onSubmit, onDelete, onAdd, onUpload, onStatusChange, onMove, onReload, onExit }) {
  const ordered = [...items].sort((a, b) => Number(a.sort || 0) - Number(b.sort || 0))
  function openEdit(item) { setEditing(item.id); setForm({ ...emptyBanner, ...item }) }
  function closeEditor() { setEditing(null); setForm(emptyBanner) }
  return <div className="admin-content banner-content"><div className="admin-panel collection-panel banner-panel"><div className="admin-panel-head"><div><span className="admin-eyebrow">MINIPROGRAM / HOME BANNERS</span><h2>小程序首页 Banner</h2><p>网站首页与小程序首页共用同一组 Banner。</p></div><div className="admin-panel-head-actions"><ReloadButton onReload={onReload} /><span className="admin-muted">共 {ordered.length} 条</span><button className="admin-primary small" onClick={onAdd}><Plus size={15} />新增 Banner</button></div></div><div className="banner-context"><div><span className="banner-context-mark"><Smartphone size={15} /></span><div><strong>小程序内容工作台</strong><p>排序、启用状态和图片在网站与小程序首页同步生效。</p></div></div><span className="banner-context-badge">网站与小程序共用</span></div><div className="admin-table-wrap"><table className="admin-table banner-table"><thead><tr><th>Banner</th><th>替代文案</th><th>排序</th><th>状态</th><th>更新时间</th><th>操作</th></tr></thead><tbody>{ordered.map((item, index) => <tr key={item.id}><td><div className="banner-table-title"><img src={assetPath(item.image)} alt="" loading="lazy" decoding="async" /><span><strong>{item.title || '未命名 Banner'}</strong><small>{item.id}</small></span></div></td><td>{item.alt || '—'}</td><td><div className="banner-order"><button type="button" onClick={() => onMove(item, -1)} disabled={index === 0} aria-label="上移"><ChevronUp size={14} /></button><b>{item.sort ?? index + 1}</b><button type="button" onClick={() => onMove(item, 1)} disabled={index === ordered.length - 1} aria-label="下移"><ChevronDown size={14} /></button></div></td><td><StatusSelect value={item.enabled === false ? 'unpublished' : 'published'} options={publicationStatusOptions} onChange={(status) => onStatusChange(item.id, status)} /></td><td>{adminDate(item.updatedAt)}</td><td><div className="table-actions"><button onClick={() => openEdit(item)}>编辑</button><button className="danger" onClick={() => onDelete(item.id)}><Trash2 size={14} />删除</button></div></td></tr>)}</tbody></table>{!ordered.length && <div className="banner-empty"><Smartphone size={22} /><strong>还没有小程序首页 Banner</strong><span>添加第一张 Banner，发布后会显示在小程序首页。</span><button className="admin-secondary small" onClick={onAdd}><Plus size={14} />新增 Banner</button></div>}</div></div>{editing && <AdminEditorPage title={editing === 'new' ? '新增小程序首页 Banner' : '编辑小程序首页 Banner'} onClose={closeEditor}><form className="admin-form banner-form" onSubmit={onSubmit}><div className="banner-form-intro"><span>SHARED HOME BANNER</span><p>这张图片会同时展示在网站首页与小程序首页。</p></div><div className="admin-form-grid"><label>Banner 标题<input required maxLength="120" value={form.title || ''} placeholder="如：雅典城市与景点导览" onChange={(e) => setForm({ ...form, title: e.target.value })} /></label><label>排序<input required type="number" min="1" max="9999" value={form.sort ?? 1} onChange={(e) => setForm({ ...form, sort: Number(e.target.value) || 1 })} /></label></div><label>图片替代文案<input required maxLength="180" value={form.alt || ''} placeholder="用于无障碍阅读与图片加载失败提示" onChange={(e) => setForm({ ...form, alt: e.target.value })} /></label><ImageUploadField label="Banner 图片" value={form.image} onChange={(image) => setForm({ ...form, image })} onUpload={(file) => onUpload(file, 'mp-banner')} required hint="建议使用 3:2 或 16:9 横图；支持 PNG、JPG、WebP，单张不超过 6MB" /><label>发布状态<select value={form.enabled === false ? 'unpublished' : 'published'} onChange={(e) => setForm({ ...form, enabled: e.target.value === 'published' })}><option value="published">启用</option><option value="unpublished">停用</option></select></label><button className="admin-primary" type="submit"><Save size={15} />保存 Banner</button></form></AdminEditorPage>}</div>
}
...
const [destinationTypes, setDestinationTypes] = useState([])
  ...
  const [miniProgramBanners, setMiniProgramBanners] = useState([])
  const [settings, setSettings] = useState(defaultSettings)
  const [homeBannersSaving, setHomeBannersSaving] = useState(false)
  ...
  async function load() {
    ...
    try {
      ...
      const [nextStats, nextRoutes, nextDestinations, nextCities, nextAttractions, nextSampleItineraries, nextCustomTrips, nextLeads, nextGuideBookings, nextMiniProgramBookings, nextSettings, nextMiniProgramUsers, nextMiniProgramTravelers, nextMiniProgramDocuments, nextMiniProgramCoupons, nextCountries, nextGuides, nextDestinationTypes, nextMiniProgramOrders, nextHomeBanners, nextMiniProgramBanners] = results.map((result) => result.status === 'fulfilled' ? result.value : null)
      ...
      if (nextHomeBanners) setHomeBanners(nextHomeBanners.items || nextHomeBanners)
      else if (nextSettings) setHomeBanners(Array.isArray(nextSettings.homeBanners) ? nextSettings.homeBanners : [])
      if (nextMiniProgramBanners) setMiniProgramBanners(nextMiniProgramBanners.items || [])
  ...
  function localUpdate(key, updater) { const current = localSeed() || { routes: [], destinations: [], leads: [], home: { banners: [] }, settings }; const next = { ...current, [key]: updater(current[key]) }; saveLocal(next); applyData(next) }
  function localUpdateSharedBanners(updater) { const current = localSeed() || { home: {}, settings: {} }; const banners = updater(current.homeBanners || current.settings?.homeBanners || current.home?.banners || []); const next = { ...current, homeBanners: banners, home: { ...(current.home || {}), banners } }; saveLocal(next); applyData(next) }
  ...
  async function saveCity(cityId, payload) {
    try {
      if (localMode) localUpdate('cities', (items) => items.map((item) => item.id === cityId ? { ...item, ...payload, id: cityId } : item))
      else await callApi(`/admin/cities/${encodeURIComponent(cityId)}`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify(payload) })
      await load(); notify('城市价格已保存')
    } catch (error) { notify(error.message) }
  }
  ...
  async function saveBanner(e) {
    ...
    try {
      ...
      if (localMode) localUpdateSharedBanners((banners) => isEdit ? banners.map((item) => item.id === editing ? { ...item, ...payload, id: editing, updatedAt: new Date().toISOString() } : item) : [...banners, { ...payload, id: localId('banner'), updatedAt: new Date().toISOString() }])
      else await callApi(isEdit ? `/admin/miniprogram-home-banners/${editing}` : '/admin/miniprogram-home-banners', { method: isEdit ? 'PATCH' : 'POST', headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify(payload) })
      await load(); setEditing(null); setBannerForm(emptyBanner); notify('小程序首页 Banner 已保存')
    } catch (error) { notify(error.message) }
  }
  async function updateBannerStatus(itemId, status) {
    const payload = { enabled: status === 'published' }
    try { if (localMode) localUpdateSharedBanners((banners) => banners.map((item) => item.id === itemId ? { ...item, ...payload, updatedAt: new Date().toISOString() } : item)); else await callApi(`/admin/miniprogram-home-banners/${itemId}`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify(payload) }); await load(); notify('Banner 状态已更新') } catch (error) { notify(error.message) }
  }
  async function removeBanner(itemId) {
    if (!window.confirm('确定删除这张首页 Banner 吗？删除后会同时影响网站和小程序首页。')) return
    try { if (localMode) localUpdateSharedBanners((banners) => banners.filter((item) => item.id !== itemId)); else await callApi(`/admin/miniprogram-home-banners/${itemId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }); await load(); notify('小程序首页 Banner 已删除') } catch (error) { notify(error.message) }
  }
  async function moveBanner(item, offset) {
    const ordered = [...miniProgramBanners].sort((a, b) => Number(a.sort || 0) - Number(b.sort || 0)); const index = ordered.findIndex((entry) => entry.id === item.id); const target = index + offset
    if (index < 0 || target < 0 || target >= ordered.length) return
    const next = [...ordered]; [next[index], next[target]] = [next[target], next[index]]
    try {
      if (localMode) localUpdateSharedBanners(() => next.map((entry, entryIndex) => ({ ...entry, sort: entryIndex + 1, updatedAt: new Date().toISOString() })))
      else { await Promise.all(next.map((entry, entryIndex) => callApi(`/admin/miniprogram-home-banners/${entry.id}`, { method: 'PATCH', headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify({ sort: entryIndex + 1 }) }))) }
      await load(); notify('Banner 排序已更新')
    } catch (error) { notify(error.message) }
  }
  ...
  async function saveSettings(e) {
    e.preventDefault()
    const { homeBanners: _homeBanners, ...settingsPayload } = settings
    try { if (localMode) localUpdate('settings', (current) => ({ ...settingsPayload, homeBanners: current.homeBanners || [] })); else await callApi('/admin/settings', { method: 'PATCH', headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify(settingsPayload) }); await load(); notify('站点配置已保存') } catch (error) { notify(error.message) }
  }
  async function saveHomeBanners(next) {
    setHomeBannersSaving(true)
    try {
      if (localMode) localUpdateSharedBanners(() => next)
      else await callApi('/admin/home-banners', { method: 'PUT', headers: { Authorization: `Bearer ${token}` }, body: JSON.stringify({ items: next }) })
      await load(); notify('首页 Banner 已保存')
    } catch (error) { notify(error.message) } finally { setHomeBannersSaving(false) }
  }
...
function CityPricingPanel({ cities, onSave, onReload }) {
  const [editing, setEditing] = useState(null)
  const [form, setForm] = useState({ priceCny: 0.01, currency: 'CNY' })
  function open(item) { setEditing(item.id); setForm({ priceCny: Number(item.priceCny ?? 0), currency: 'CNY' }) }
  async function submit(event) {
    event.preventDefault()
    const priceCny = Number(form.priceCny)
    if (!Number.isFinite(priceCny) || priceCny < 0) return
    await onSave(editing, { priceCny, currency: 'CNY', price: `¥${priceCny.toFixed(2)}` })
    setEditing(null)
  }
  return <div className="admin-content"><div className="admin-panel collection-panel"><div className="admin-panel-head"><div><span className="admin-eyebrow">CITY GUIDES / CNY PRICING</span><h2>城市导览包价格</h2><p className="admin-muted">面向国内用户统一使用人民币；MpApp 只消费 priceCny，不做汇率换算。</p></div><div className="admin-panel-head-actions"><ReloadButton onReload={onReload} /><span className="admin-muted">共 {cities.length} 个城市</span></div></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>城市</th><th>城市 ID</th><th>priceCny</th><th>currency</th><th>兼容显示</th><th>操作</th></tr></thead><tbody>{cities.map((city) => <tr key={city.id}><td><strong>{city.name}</strong><small>{city.nameEn || city.country || '希腊'}</small></td><td>{city.id}</td><td>{Number.isFinite(Number(city.priceCny)) ? `¥${Number(city.priceCny).toFixed(2)}` : '未配置'}</td><td>{city.currency || '未配置'}</td><td>{city.price || '—'}</td><td><button className="table-edit-button" onClick={() => open(city)}>编辑价格</button></td></tr>)}</tbody></table>{!cities.length && <div className="admin-empty">暂无城市数据。</div>}</div></div>{editing && <AdminEditorPage title="编辑城市导览包价格" onClose={() => setEditing(null)}><form className="admin-form" onSubmit={submit}><div className="admin-form-grid"><label>城市 ID<input value={editing} readOnly /></label><label>货币<select value="CNY" disabled><option value="CNY">CNY · 人民币</option></select></label></div><label>人民币价格（priceCny）<input required type="number" min="0" step="0.01" value={form.priceCny} onChange={(event) => setForm({ ...form, priceCny: event.target.value })} /></label><p className="admin-muted">保存后同步写入 priceCny、currency=CNY，并将兼容字段 price 更新为人民币显示文本。</p><button className="admin-primary" type="submit"><Save size={15} />保存价格</button></form></AdminEditorPage>}</div>
}
...
function HomeBannersEditor({ value, onChange, onUpload, onSave, saving = false }) {
  const items = Array.isArray(value) ? value : []
